...
class AlbumsHandler {
  ...
  async postCoverHandler(request, h) {
    ...
    if (!cover || !cover.hapi || !cover.hapi.headers) {
      throw new InvariantError('cover harus diisi');
    }

    this._validator.validateCoverHeaders(cover.hapi);
  ...
  async postLikeAlbumHandler(request, h) {
    ...
    await this._likesService.verifyUserHasNotLikedAlbum(userId, albumId);

    await this._likesService.likeAlbum(userId, albumId);
    await this._cacheService.delete(`album-likes:${albumId}`);

    const response = h.response({ status: 'success', message: 'Berhasil menyukai album' });
    response.code(201);
    return response;
  }
